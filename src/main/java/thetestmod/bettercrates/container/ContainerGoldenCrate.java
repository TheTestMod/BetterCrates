...
public class ContainerGoldenCrate extends Container {
	
	//add slot
		public ContainerGoldenCrate(InventoryPlayer playerInv, final TileEntityGoldenCrate crate) {
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 9; j++) {
					addSlotToContainer(new Slot(playerInv, j + i * 9 + 9, 12/**ширина*/ + j * 18, 174/**высота*/ + i * 18));
				}
			}
			
		
			for (int k = 0; k < 9; k++) {
				addSlotToContainer(new Slot(playerInv, k, 12/**ширина*/ + k * 18, 232/**высота*/));
			}

...
public class ContainerWoodenCrate extends Container {
	...
	//add slot
		public ContainerWoodenCrate(InventoryPlayer playerInv, final TileEntityWoodenCrate crate) {
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 9; j++) {
					addSlotToContainer(new Slot(playerInv, j + i * 9 + 9, 12/**ширина*/ + j * 18, 84/**высота*/ + i * 18));
				}
			}
			
		
			for (int k = 0; k < 9; k++) {
				addSlotToContainer(new Slot(playerInv, k, 12/**ширина*/ + k * 18, 142/**высота*/));
			}

...
public class DiamondCrate extends BlockTileEntity<TileEntityDiamondCrate> {
	
	public DiamondCrate(String name, Material material, SoundType sound) {
	...
	@Override
	public Class<TileEntityDiamondCrate> getTileEntityClass() {
		return TileEntityDiamondCrate.class;
	}
	
	@Nullable
	@Override
	public TileEntityDiamondCrate createTileEntity(World world, IBlockState state) {
		return new TileEntityDiamondCrate();
	}
	//logic
	@Override
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			...
			TileEntityDiamondCrate tile = getTileEntity(world, pos);
			IItemHandler itemHandler = tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side);
			player.openGui(BetterCrates.instance, GuiRegistry.DIAMOND_CRATE, world, pos.getX(), pos.getY(), pos.getZ());
  		}
  		return true;
	}
    ...
    public List<ItemStack> getDrops(IBlockAccess w, BlockPos pos, IBlockState s, int f) {
        ...
        if (te instanceof TileEntityDiamondCrate) {
            ItemStack stack = new ItemStack(Item.getItemFromBlock(this));
            NBTTagCompound tagCompound = new NBTTagCompound();
            ((TileEntityDiamondCrate) te).writeRestorableToNBT(tagCompound);
    ...
    public void onBlockPlacedBy(World w, BlockPos pos, IBlockState s, EntityLivingBase p, ItemStack is) {
        ...
        if (tagCompound != null) {
            TileEntity te = w.getTileEntity(pos);
            if (te instanceof TileEntityDiamondCrate)
                ((TileEntityDiamondCrate) te).readRestorableFromNBT(tagCompound);
        }
    }
    @Override

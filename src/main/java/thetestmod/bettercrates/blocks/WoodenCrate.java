...
public class WoodenCrate extends BlockTileEntity<TileEntityWoodenCrate> {

	public WoodenCrate(String name, Material material, SoundType sound) {
	...
	@Override
	public Class<TileEntityWoodenCrate> getTileEntityClass() {
		return TileEntityWoodenCrate.class;
	}

	@Nullable
	@Override
	public TileEntityWoodenCrate createTileEntity(World world, IBlockState state) {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (!world.isRemote) {
			...
			TileEntityWoodenCrate tile = getTileEntity(world, pos);
			IItemHandler itemHandler = tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side);
			player.openGui(BetterCrates.instance, GuiRegistry.WOODEN_CRATE, world, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public List<ItemStack> getDrops(IBlockAccess w, BlockPos pos, IBlockState s, int f) {
		...
		if (te instanceof TileEntityWoodenCrate) {
			...
			((TileEntityWoodenCrate) te).writeRestorableToNBT(tagCompound);
	...
	public void onBlockPlacedBy(World w, BlockPos pos, IBlockState s, EntityLivingBase p, ItemStack is) {
		...
		if (tagCompound != null) {
			TileEntity te = w.getTileEntity(pos);
			if (te instanceof TileEntityWoodenCrate)
				((TileEntityWoodenCrate) te).readRestorableFromNBT(tagCompound);
		}
	}

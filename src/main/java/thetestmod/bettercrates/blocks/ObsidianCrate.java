...
public class ObsidianCrate extends BlockTileEntity<TileEntityObsidianCrate> {
	
	public ObsidianCrate(String name, Material material, SoundType sound) {
	...
	@Override
	public Class<TileEntityObsidianCrate> getTileEntityClass() {
		return TileEntityObsidianCrate.class;
	}
	
	@Nullable
	@Override
	public TileEntityObsidianCrate createTileEntity(World world, IBlockState state) {
		return new TileEntityObsidianCrate();
	}
	//logic
	@Override
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			...
			TileEntityObsidianCrate tile = getTileEntity(world, pos);
			IItemHandler itemHandler = tile.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side);
			player.openGui(BetterCrates.instance, GuiRegistry.OBSIDIAN_CRATE, world, pos.getX(), pos.getY(), pos.getZ());
  		}
  		return true;
	}
    ...
    public List<ItemStack> getDrops(IBlockAccess w, BlockPos pos, IBlockState s, int f) {
        ...
        if (te instanceof TileEntityObsidianCrate) {
            ItemStack stack = new ItemStack(Item.getItemFromBlock(this));
            NBTTagCompound tagCompound = new NBTTagCompound();
            ((TileEntityObsidianCrate) te).writeRestorableToNBT(tagCompound);
    ...
    public void onBlockPlacedBy(World w, BlockPos pos, IBlockState s, EntityLivingBase p, ItemStack is) {
        ...
        if (tagCompound != null) {
            TileEntity te = w.getTileEntity(pos);
            if (te instanceof TileEntityObsidianCrate)
                ((TileEntityObsidianCrate) te).readRestorableFromNBT(tagCompound);
        }
    }
    @Override
	...
	public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("item.crate_base2.tooltip"));
	}
